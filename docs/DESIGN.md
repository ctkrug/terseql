...
- **Synth SFX (WebAudio-generated, no audio files):**
  - _keystroke_: near-silent 20ms tick, filtered noise, only on byte-count digit change (throttled)
  - _run_: short rising sine blip, 80ms
  - _fail_: low detuned square-wave buzz, 150ms
  - _pass_: two-note ascending sine chime, 220ms
  - _win (submit)_: three-note major arpeggio, 400ms
